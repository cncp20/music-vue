<template>
    <div class="songs">
        ...
        <div class="songs-list" v-if="!isLoading">
            <img :src="rankList.topinfo.pic_h5" class="logo"/>
            <p class="dissname">{{rankList.topinfo.ListName}}</p>
            <div class="song-item" v-for="(item, index) in rankList.songlist" key="index" @click="pushToPlayListAndPlay(item.data)">
                <span>{{index + 1 }}</span>
                <div class="song-info">
                    <p class="title">{{item.data.songname}}</p>
                    <p class="subtitle">{{item.data.singer[0].name}}</p>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        computed:mapGetters(["rankList", "isLoading"])
    }
</script>
